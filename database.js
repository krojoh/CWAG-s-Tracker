...
async function loadDogInfoData() {
    return new Promise((resolve) => {
        const timeoutId = setTimeout(() => {
            ...
            resolve(false);
        }, appConfig.dogInfoTimeout);

        fetch(appConfig.dogInfoFile, {
            method: 'GET',
            cache: 'no-cache'
        })
        .then(response => {
...
async function loadTitlePlacementData() {
    return new Promise((resolve) => {
        ...
        const timeoutId = setTimeout(() => {
            ...
            resolve(false);
        }, appConfig.titlePlacementTimeout);

        fetch(appConfig.titlePlacementFile, {
            method: 'GET',
            cache: 'no-cache'
        })
        .then(response => {
...
function showFileUpload() {
    document.getElementById('summaryContent').innerHTML = `
        <div class="file-upload-section">
            ...
            <p>Select your Excel file to load the trialing data:</p>
            
            <div class="file-upload-area">
                <input type="file" id="manualDataFile" accept=".xlsx,.xls,.csv">
                <br>
                <button onclick="loadManualFile()" class="upload-btn">
                    📤 Load File
                </button>
            </div>
            
            <button onclick="loadDataFromFile()" class="retry-btn">
